...
def generator_4_multiple_types(batch_size=32):
    X = np.zeros((batch_size, setting.HEIGHT, setting.WIDTH, 1), dtype=np.uint8)
    ...
    while True:
        for i in range(batch_size):
            ...
            image = np.asarray(generator.generate_image(random_str))
            X[i] = np.expand_dims(rgb2gray(image), axis=2)
